...
class HTML_generator():
   ...
   def html_index(self):
      ...
      self.make_index('genus', length=1)

      # Make html genus_de index
      self.make_index('genus_de', 'species_de', length=1)
   ...
   def make_index(self, index1, index2='', length=100):
      ...
      for species in self.database['species']:
         name = self.getindex(species, index1, index2)
      ...
      for key in keys:
         sorted_species[key] = \
            sorted(sorted_species[key], \
                   key=lambda sp: self.getindex(sp, index1, index2))
